...
def main() -> None:
    ...
    required_cols = ["Periode", "Canton_ISO2", "Groupe_de_couts", "Prestations_brutes_par_assure"]
    ...
    df_model = df[~df["Groupe_de_couts"].isin([
        "Total",
    ...
    print(f"   Period range: {df_model['Periode'].min()} to {df_model['Periode'].max()}")
    
    # Check for duplicates
    duplicates = df_model.duplicated(subset=["Periode", "Canton_ISO2", "Groupe_de_couts"])
    if duplicates.any():
        print(f"   ⚠️  Warning: {duplicates.sum()} duplicate rows found and will be handled")
